...
export type StrategyStatus = "live" | "paused" | "locked" | "draft";
...
export interface Strategy {
  ...
  description: string;
  rules: Rule[];
  risk: { perTrade: string; maxConcurrent: number };
  guardrails: { maxDrawdown: string; dailyLossCap: string; killSwitch: boolean };
  mastery: { done: number; total: number };
  perf: { trades: number; winRate: number; pnlPct: number; sharpe: number };
  since: string;
}
...
export const strategies: Strategy[] = [
  {
    ...
    rules: [
      ...
      { kind: "THEN", text: "exit at EMA(20) or −1R", tone: "neutral" },
    ],
    risk: { perTrade: "1%", maxConcurrent: 1 },
    guardrails: { maxDrawdown: "−8%", dailyLossCap: "$250", killSwitch: false },
    mastery: { done: 5, total: 5 },
    perf: { trades: 41, winRate: 63, pnlPct: 14.2, sharpe: 1.9 },
    since: "live 12 days",
  },
  {
    id: "st-nvda-mo",
    ...
    rules: [
      ...
      { kind: "AND", text: "volume > 1.5× avg" },
      { kind: "THEN", text: "LONG · risk 0.75%", tone: "profit" },
    ],
    risk: { perTrade: "0.75%", maxConcurrent: 2 },
    guardrails: { maxDrawdown: "−6%", dailyLossCap: "$200", killSwitch: false },
    mastery: { done: 5, total: 5 },
    perf: { trades: 23, winRate: 57, pnlPct: 8.6, sharpe: 1.4 },
  ...
  },
  {
    ...
    risk: { perTrade: "1%", maxConcurrent: 1 },
    guardrails: { maxDrawdown: "−5%", dailyLossCap: "$150", killSwitch: false },
    mastery: { done: 3, total: 5 },
    perf: { trades: 0, winRate: 0, pnlPct: 0, sharpe: 0 },
    since: "draft #3",
  },
];
...
export const activityFeed: ActivityEvent[] = [
  ...
  { id: "a5", time: "Yesterday", kind: "blocked", strategy: "Mean-reversion · BTC", text: "Second entry blocked — max concurrent positions (1) reached." },
...
export const brokers: Broker[] = [
  ...
  { id: "ibkr", name: "Interactive Brokers", glyph: "IB", kind: "Stocks & FX" },
  { id: "oanda", name: "OANDA", glyph: "O", kind: "Forex" },
];

/* ————— Strategy builder palette ————— */
export const builderPalette = {
  indicators: ["RSI(14)", "EMA(20)", "EMA(50)", "EMA(200)", "MACD", "ATR", "Volume", "VWAP", "Bollinger"],
